...
function Profile() {
	...
	const [joinedDate, setJoinDate] = useState(new Date(parseInt(currentUser.createdAt)))
	...
	useEffect(() => {
		;(async () => {
			try {
				...
				// setAllTweetsQuery(allTweetsQuery)
				...
				alert("Error at line 67	 of profile, check console")
				console.log(error)
			}
		})()
	}, [])
	...
	return (
		<div className='h-full xl:basis-2/4 border border-gray-200 relative'>
			...
			{tweetsOfCurrentUser.map(tweet => {
				return <Tweet key={tweet.date} tweet={tweet}></Tweet>
			})}
		</div>
	)
}
